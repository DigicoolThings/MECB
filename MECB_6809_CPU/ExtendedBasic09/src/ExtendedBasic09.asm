...
;
UART      EQU  $C008		; MC6850 ACIA (UART) base address (Originally $A000)
RECEV     EQU  UART+1          
TRANS     EQU  UART+1          
USTAT     EQU  UART            
UCTRL     EQU  UART            
...
L8A91     LDX  TXTTAB        ;GET START OF BASIC PROGRAM 
          LEAX -1,X        ;MOVE POINTER BACK ONE 
L8A95     LEAX 1,X         ;MOVE POINTER UP ONE 
          BSR  L8A86         ;RETURN IF END OF PROGRAM 
L8A99     LEAX 3,X         ;SKIP OVER NEXT LINE ADDRESS AND LINE NUMBER 
L8A9B     LEAX 1,X         ;MOVE POINTER TO NEXT CHARACTER 
...
L8AE1     LDX  CHARAD        ;*GET CURRENT INPUT POINTER 
          LEAX 1,X         ;*AND BUMP IT ONE 
L8AE5     BSR  L8A86         ;RETURN IF END OF PROGRAM 
          LEAX 2,X         ;SKIP PAST NEXT LINE ADDRESS 
L8AE9     LEAX 1,X         ;ADVANCE POINTER BY ONE 
...
L8B24     LDA  ,-X           ;GET PREVIOUS INPUT CHARACTER 
          ...
          SUBB 1,S         ;SUBTRACT PRE-NUMERIC POINTER LS BYTE 
...
L8B7B     LDX  TXTTAB        ;POINT X TO START OF BASIC PROGRAM 
          LEAX -1,X        ;MOVE POINTER BACK ONE 
L8B7F     LEAX 1,X         ;MOVE POINTER UP ONE 
          LDD  2,X         ;GET ADDRESS OF NEXT LINE 
          STD  CURLIN        ;SAVE IT IN CURLIN 
          JSR  L8A86         ;RETURN IF END OF PROGRAM 
          LEAX 3,X         ;SKIP OVER ADDRESS OF NEXT LINE AND 1ST BYTE OF LINE NUMBER 
L8B8A     LEAX 1,X         ;MOVE POINTER UP ONE 
