;******************************************************************************
;	MECBChristmas09_VDP_FrameIRQ.asm
;
;	A simple 6809 test for the MECB TMS9929A VDP Card and IRQ Interrupts.
;
...
;
FrameCount 	EQU	$80	; This location holds Frame Count until next Backdrop Color change.
BackdropClr	EQU	$81	; This location holds the current Backdrop Color
;
; Main Entry Point
		ORG	Entry
...
; Initialize FrameCount
		LDA 	#$32		; 1 Second initial pause at 50Hz Frame Rate (PAL)
		STA 	FrameCount
; Initialize BackdropClr
		LDA 	#$01		; Initialise Backdrop Color
		STA 	BackdropClr
; Setup IRQ Handler
		LEAX	irqHandler,PCR
		LDA	#IRQ_CODE
...
; Clear CC IRQ Flag - Enable IRQ Interrupts
		ANDCC	#$EF		
; Finally, Enable VDP Interrupts (as well as Display Enabled)
...
;
irqHandler	
; Function:	Handler for VDP Frame IRQ - Change Backdrop Color every 50 frames
		DEC	FrameCount
		BNE	irqReturn
		LDA 	#$32		; Reset FrameCount for 50 frames
		STA 	FrameCount
		INC	BackdropClr
		LDA	BackdropClr
		...
		STA	BackdropClr
SkipRst		LDB	#7		; Write Backdrop Color (in A) to VDP Register 7
		LBSR	writeRegisterByteVDP
;
irqReturn	LDA	VDP_REGISTER	; Dummy Read VDP Status Register to reset Frame Interrupt
		RTI
;
